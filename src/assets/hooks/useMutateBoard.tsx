...
const useBoardMutation = () => {
  ...
  const editBoardNameMutation = useMutation({
    mutationFn: async (data: { boardId: string; name: string }) => {
      const res = await axios.put<BoardData>(
        `https://${process.env.HOST_URL}/user/boards/${data.boardId}`,
        { name: data.name },
        { headers: authHeader() }
      );
      return res.data;
  ...
  const editStatusMutation = useMutation({
    mutationFn: async (data: {
      ...
      const res = await axios.put<StatusCodes>(
        `https://${process.env.HOST_URL}/user/boards/${data.boardId}/status_codes/${data.statusId}`,
        { name: data.name },
        { headers: authHeader() }
      );
      return res.data;
  ...
  const editStatusColorMutation = useMutation({
    mutationFn: async (data: {
      ...
      const res = await axios.put<StatusCodes>(
        `https://${process.env.HOST_URL}/user/boards/${data.boardId}/status_codes/${data.statusId}/color`,
  ...
  const createBoardMutation = useMutation({
    mutationFn: async (data: {
      ...
      return await axios
        .post<BoardData>(
          `https://${process.env.HOST_URL}/user/boards/create`,
          {
            name: data.name,
  ...
  const addStatusMutation = useMutation({
    mutationFn: async (data: { boardId: string; name: string }) => {
      const res = await axios.post<StatusCodes>(
        `https://${process.env.HOST_URL}/user/boards/${data.boardId}/status_codes`,
        {
  ...
  const deleteBoardMutation = useMutation({
    mutationFn: (boardId: string) => {
      return axios.delete<BoardData>(
        `https://${process.env.HOST_URL}/user/boards/${boardId}`,
        { headers: authHeader() }
      );
    },
    onSuccess: () => {
  ...
  const shareBoardMutation = useMutation({
    mutationFn: (data: { userId: number; boardId: number }) => {
      return axios.put<UserType>(
        `https://${process.env.HOST_URL}/user/boards/assign/${data.userId}`,
        { boardId: data.boardId },
        { headers: authHeader() }
      );
    },
