...
const useBoardMutation = () => {
  ...
  const editBoardNameMutation = useMutation({
    mutationFn: async (data: { boardId: string; name: string }) => {
      const res = await axios.put<BoardData>(
        `https://${import.meta.env.VITE_HOST_URL}/boards/${data.boardId}`,
        { name: data.name },
        { headers: authHeader() }
      );
      return res.data;
  ...
  const createBoardMutation = useMutation({
    mutationFn: async (data: {
      ...
      return await axios
        .post<BoardData>(
          `https://${import.meta.env.VITE_HOST_URL}/boards/create`,
          {
            name: data.name,
  ...
  const deleteBoardMutation = useMutation({
    mutationFn: (boardId: string) => {
      return axios.delete<BoardData>(
        `https://${import.meta.env.VITE_HOST_URL}/boards/${boardId}`,
        { headers: authHeader() }
      );
    },
    onSuccess: () => {
  ...
  const shareBoardMutation = useMutation({
    mutationFn: (data: { userId: number; boardId: number }) => {
      return axios.put<UserType>(
        `https://${import.meta.env.VITE_HOST_URL}/boards/assign/${data.userId}`,
        { boardId: data.boardId },
        { headers: authHeader() }
      );
    },
