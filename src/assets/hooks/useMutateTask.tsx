  ...
  const createTaskMutation = useMutation({
    mutationFn: async (data: CreateTaskData) => {
      const response = await axios.post<TaskData>(
        `http://${process.env.API_URL}/user/boards/${data.boardId}/tasks`,
        {
          title: data.title,
  ...
  const editTaskMutation = useMutation({
    mutationFn: async (data: EditTaskData) => {
      const response = await axios.put<TaskData>(
        `http://${process.env.API_URL}/user/boards/${data.boardId}/tasks/${data.taskId}`,
        {
          title: data.title,
          description: data.description,
          statusId: data.statusId,
  ...
  const addSubtaskMutation = useMutation({
    mutationFn: async (data: { taskId: number; title: string }) => {
      const response = await axios.post<SubtaskData>(
        `http://${process.env.API_URL}/user/tasks/${data.taskId}/subtasks`,
        {
          taskId: data.taskId,
          title: data.title,
        },
  ...
  const editSubtaskMutation = useMutation({
    mutationFn: async (data: { taskId: number; id: number; title: string }) => {
      const response = await axios.put<SubtaskData>(
        `http://${process.env.API_URL}/user/tasks/${data.taskId}/subtasks/${data.id}`,
        {
          title: data.title,
          taskId: data.taskId,
        },
  ...
  const editStatusMutation = useMutation({
    mutationFn: async (data: {
      ...
      const response = await axios.put(
        `http://${process.env.API_URL}/user/boards/${data.boardId}/tasks/${data.taskId}/new_status/${data.statusId}`,
        {},
        { headers: authHeader() }
      );
      return response;
