...
const useUserMutation = () => {
  ...
  const denyUserAccessMutation = useMutation({
    mutationFn: async (data: { userId: number; boardId: number }) => {
      try {
        const res = await axios.put<UserType>(
          `http://localhost:8080/user/boards/${data.boardId}/user_access`,
          { userId: data.userId },
          { headers: authHeader() }
        );
        return res;
  ...
  const changeUserPasswordMutation = useMutation({
    mutationFn: async (data: {
      ...
      try {
        const res = await axios.put<UserType>(
          `http://localhost:8080/user/${data.id}/password_change`,
          {
  ...
  const removeBoardAccessMutation = useMutation({
    mutationFn: async (data: { boardId: number }) => {
      try {
        const res = await axios.put<UserType>(
          `http://localhost:8080/user/boards/${data.boardId}/abandon_access`,
          {},
          { headers: authHeader() }
        );
        return res;
    ...
    mutationFn: async (data: {
      ...
      try {
        const res = await axios.delete<UserType>(
          `http://localhost:8080/user/${data.userId}/delete`,
          {
            data: {
              username: data.username,
              email: data.email,
